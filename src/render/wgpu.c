...
int instanceWait(WGPUFuture future) {
    ...
    WGPUWaitStatus status = wgpuInstanceWaitAny(renderContext.instance, 1, &wait, 0);
...
WGPUAdapter requestAdapter() {
    ...
    if (instanceWait(wgpuInstanceRequestAdapter(
...
int configureSurface() {
    ...
    wgpuSurfaceGetCapabilities(renderContext.surface, renderContext.adapter, &capabilities);
...
int initWGPU() {
    ...
    if ((renderContext.surface = wgpuGlfwCreateSurfaceForWindow(renderContext.instance, renderContext.window)) == 0) {
        finishWGPU();
        return -1;
    }
    if (configureSurface()) {
